...
This KMIP server is made only for cryptctl2's own KMIP client, hence a lot of validation work are skipped intentionally.
Normally KMIP service is capable of handling more than one requests per connection, but cryptctl2's own KMIP client only
...
func (srv *KMIPServer) HandleRequest(req structure.SerialisedItem, conn net.Conn) (err error) {
	defer func() {
		/*
			The KMIP request only comes from cryptctl2 program itself, so all type assertions and attributes
