...
func TestEncryptDecrypt(t *testing.T) {
	...
	// Start an RPC server
	keydbDir := "/tmp/cryptctl2-encrypttest"
	...
	sysconf.Set(keyserv.SRV_CONF_MAIL_FROM_ADDR, "cryptctl2@localhost")
	...
	os.RemoveAll("/cryptctl2-encrypttest")
	srcDir0 := "/cryptctl2-encrypttest/secret0"
	srcDir1 := "/cryptctl2-encrypttest/secret1"
	movedSrcDir0 := "/cryptctl2-encrypttest/" + SRC_DIR_NEW_NAME_PREFIX + "secret0"
	movedSrcDir1 := "/cryptctl2-encrypttest/" + SRC_DIR_NEW_NAME_PREFIX + "secret1"
	loDisk0 := "/cryptctl2-encrypttest/disk0"
	loDisk1 := "/cryptctl2-encrypttest/disk1"
	loDiskSrc1 := "/cryptctl2-encrypttest/disk-src1"
	loop0Crypt := MakeDeviceMapperName("/dev/loop0")
	loop1Crypt := MakeDeviceMapperName("/dev/loop1")
	loop0Mount, err := ioutil.TempDir("", "cryptctl2-test")
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
		...
		if err := os.RemoveAll("/cryptctl2-encrypttest"); err != nil {
			fmt.Fprintln(os.Stderr, err)
		}
	}()
	...
	*/
	...
	checkSecret0 := func() {
		if st, err := fs.CryptStatus(loop0Crypt); err != nil || st.Device != "/dev/loop0" ||
			st.KeySize != fs.LUKS_KEY_SIZE_I || st.Cipher != fs.LUKS_CIPHER || st.Loop != "/cryptctl2-encrypttest/disk0" {
